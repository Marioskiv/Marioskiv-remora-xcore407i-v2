...
#include "remora-core/remora.h"
    ...
    #include "remora-hal/STM32F4_NativeEthUDPComms.h"
#elif defined(USE_NATIVE_ETHERNET)
    #include "remora-hal/STM32F4_NativeEthComms.h"
#elif defined(ETH_CTRL)
...
int main(void)
{
    ...
    #ifdef USE_NATIVE_ETHERNET_UDP
        printf("Using Native Ethernet UDP (DP83848I RMII PHY)\n");
        comms = std::make_unique<STM32F4_NativeEthUDPComms>(&rxData, &txData);
    #elif defined(USE_NATIVE_ETHERNET)
        printf("Using Native Ethernet TCP (DP83848I RMII PHY)\n");
        comms = std::make_unique<STM32F4_NativeEthComms>(&rxData, &txData);
    #elif defined(ETH_CTRL)
        #ifndef WIZ_RST
            #error "Please configure your WIZ_RST pin in platformio.ini"
        #endif
        printf("Using W5500 SPI Ethernet\n");
        comms = std::make_unique<STM32F4_EthComms>(&rxData, &txData, SPI_MOSI, SPI_MISO, SPI_CLK, SPI_CS, WIZ_RST);
    #else
        printf("Using SPI Communication\n");
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  // PLLR not available on STM32F407
