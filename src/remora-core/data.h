...
typedef union txData_t
{
  ...
  {
    int32_t header;
    int32_t jointFeedback[Config::joints];	  // Encoder counts (actual position)
    float processVariable[Config::variables];		     // Servo thread feedback
	uint32_t inputs;                            // Digital inputs bitfield (LinuxCNC expects 32-bit)
	uint16_t NVMPGinputs;                       // Optional NV MPG inputs (10 bits used)
  };
