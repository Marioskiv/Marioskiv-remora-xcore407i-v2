...
std::shared_ptr<Module> DigitalPin::create(const JsonObject& config, Remora* instance) {
	...
	volatile uint32_t* ptrData = (!strcmp(mode, "Output")) ? &instance->getRxData()->outputs : &instance->getTxData()->inputs;
...
DigitalPin::DigitalPin(volatile uint32_t& _ptrData, int _mode, std::string _portAndPin, 
