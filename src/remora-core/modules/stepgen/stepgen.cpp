...
std::shared_ptr<Module> Stepgen::create(const JsonObject& config, Remora* instance)
	{
        ...
        // Use internal state for commanded step count (not part of wire protocol)
        extern InternalState gInternalState;
        volatile int32_t* ptrJointStepCount = &gInternalState.jointStepCount[joint];
	    ...
	    return std::make_unique<Stepgen>(threadFreq, joint, enable, step, dir, Config::stepBit, *ptrJointFreqCmd, *ptrJointStepCount, *ptrJointEnable, usesModulePost);
	}
 ...
 */
Stepgen::Stepgen(int32_t _threadFreq, int _jointNumber, const char* _enable, const char* _step, const char* _direction, int _stepBit, volatile int32_t& _ptrFrequencyCommand, volatile int32_t& _ptrStepCount,  volatile uint8_t& _ptrJointEnable, bool _usesModulePost)
...
void Stepgen::makePulses()
{
    ...
    if (stepNow)
    {
        ...
        *ptrStepCount = rawCount;  // Update the step count (separate from encoder feedback)
